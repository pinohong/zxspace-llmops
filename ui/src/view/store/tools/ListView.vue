  ...
  <a-spin :loading="getBuiltinToolsLoading" class="block h-full w-full">
    <div class="p-6 flex flex-col">
      ...
      <div class="flex items-center justify-between mb-6">
        <!-- 左侧分类 -->
        <div class="flex items-center gap-2">
          <a-button :type="category === 'all' ? 'secondary' : 'text'" class="rounded-lg !text-gray-700 px-3"
            @click="category = 'all'">全部
          </a-button>
          <a-button v-for="item in categories" :key="item.category"
            :type="category === item.category ? 'secondary' : 'text'" class="rounded-lg !text-gray-700 px-3"
            @click="category = item.category">
            {{ item.name }}
          </a-button>
        </div>
        <!-- 右侧搜索 -->
        <a-input-search v-model="search_word" placeholder="请输入插件名称"
      ...
      <a-row :gutter="[20, 20]" class="flex-1">
        ...
        <a-col v-for="(builtinTool, idx) in filterBuiltinTools" :key="builtinTool.name" :span="6">
          <a-card hoverable class="cursor-pointer rounded-lg" @click="showIdx = Number(idx)">
            ...
            <div class="flex items-center gap-3 mb-3">
              <!-- 左侧图标 -->
              <a-avatar :size="40" shape="square" :style="{ backgroundColor: builtinTool.background }">
                <img :src="`${apiPrefix}/builtin-tools/${builtinTool.name}/icon`" :alt="builtinTool.name" />
              </a-avatar>
              <!-- 右侧工具信息 -->
        ...
        <a-col v-if="filterBuiltinTools.length === 0" :span="24">
          <a-empty description="没有可用的内置插件" class="h-[400px] flex flex-col items-center justify-center" />
        </a-col>
      </a-row>
      ...
      <a-drawer :visible="showIdx != -1" :width="350" :footer="false" title="工具详情"
        ...
        <div v-if="showIdx != -1" class="">
          ...
          <div class="flex items-center gap-3 mb-3">
            <!-- 左侧图标 -->
            <a-avatar :size="40" shape="square" :style="{ backgroundColor: filterBuiltinTools[showIdx].background }">
              <img :src="`${apiPrefix}/builtin-tools/${filterBuiltinTools[showIdx].name}/icon`"
          ...
          <div class="flex flex-col gap-2">
            ...
            <!-- 工具列表 -->
            <a-card v-for="tool in filterBuiltinTools[showIdx].tools" :key="tool.name"
