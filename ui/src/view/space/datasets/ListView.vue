<template>
  <a-spin :loading="loading" class="block h-full w-full scrollbar-w-none overflow-scroll" @scroll="handleScroll">
    <!-- 底部知识库列表 -->
    <a-row :gutter="[20, 20]" class="flex-1">
          ...
          <div class="flex items-center gap-3 mb-3">
            ...
            <div class="flex flex-1 justify-between">
              <div class="flex flex-col">
                ...
                }" class="text-base text-gray-900 font-bold">{{ dataset.name }}
                </router-link>
              ...
              <a-dropdown position="br">
                ...
                <template #content>
                  ...
                  <a-doption class="!text-red-500" @click="() => handleDelete(dataset.id, () => loadDatasets(true))">
                    删除
                  </a-doption>
                </template>
              </a-dropdown>
      ...
      <a-col v-if="datasets.length === 0" :span="24">
        <a-empty description="没有可用的知识库" class="h-[400px] flex flex-col items-center justify-center" />
      </a-col>
    </a-row>
    <!-- 加载器 -->
    ...
    <!-- 新建/修改模态窗 -->
    <a-modal :width="520" :visible="props.createType === 'dataset' || showUpdateModal" hide-title :footer="false"
      ...
      <div class="pt-6">
        <a-form ref="formRef" :model="form" @submit="handleSubmit" layout="vertical">
          <a-form-item field="fileList" hide-label :rules="[{ required: true, message: '知识库图标不能为空' }]">
          ...
          <a-form-item field="name" label="知识库名称" asterisk-position="end"
            :rules="[{ required: true, message: '知识库名称不能为空' }]">
            <a-input v-model="form.name" placeholder="请输入知识库名称" show-word-limit :max-length="60" />
          </a-form-item>
          <a-form-item field="description" label="知识库描述" asterisk-position="end">
            <a-textarea v-model="form.description" :auto-size="{ minRows: 4, maxRows: 6 }" placeholder="请输入知识库内容的描述" />
          </a-form-item>
          <!-- 底部按钮 -->
          <div class="flex items-center justify-between">
            <div class=""></div>
...
const handleUpdate = (dataset_id: string) => {
  updateShowUpdateModal(true, async () => {
    ...
    form.value.fileList = [{
      uid: '1',
      name: '知识库图标',
      url: 'https://picsum.photos/400', //||dataset.value.icon
    }]
