<template>
  <div class="">
    <!-- 召回测试模态窗 -->
    <a-modal :width="1000" :visible="props.visible" hide-title :footer="false"
      modal-class="rounded-xl h-3/4 overflow-auto scrollbar-w-none" @cancel="hideHitTestingModal">
      <!-- 顶部标题 -->
          ...
          <div class="flex flex-col w-1/2">
            ...
            <div class="border border-blue-700 bg-blue-100 rounded-lg flex flex-col mb-6">
              ...
              <div class="flex items-center justify-between px-4 py-1.5">
                ...
                <a-button size="small" class="rounded-lg px-2" @click="retrievalSettingModalVisible = true">
                  <template #icon>
                    <icon-language />
                  </template>
              ...
              <div class="bg-white rounded-lg p-2">
                <!-- 输入框 -->
                <a-textarea v-model:model-value="hitTestingForm.query" placeholder="请输入文本，建议使用简短的陈述句" :max-length="200"
                  :auto-size="{ minRows: 6, maxRows: 6 }" class="!bg-white !border-0 mb-1" />
                <!-- 字符限制以及召回按钮 -->
                <div class="flex items-center justify-between">
                  <a-tag size="small" class="rounded text-gray-700">
                    {{ hitTestingForm.query.length }}/200
                  </a-tag>
                  <a-button :loading="hitLoading" type="primary" size="small" class="rounded-lg"
            ...
            <div class="">
              ...
              <a-table :loading="getDatasetQueriesLoading" :pagination="false" size="small" :bordered="{ wrapper: false }"
                :data="queries" @row-click="(record) => (hitTestingForm.query = record.query)">
                <template #columns>
                  <a-table-column title="数据源" data-index="source"
                    header-cell-class="text-gray-500 bg-transparent border-b font-bold" cell-class="text-gray-500"
                    :width="110" />
              ...
              <a-row v-if="hitTestingSegments.length > 0" :gutter="[16, 16]">
                <a-col v-for="segment in hitTestingSegments" :key="segment.id" :span="12">
                  <div class="p-4 bg-gray-50 rounded-lg cursor-pointer">
                    <!-- 顶部得分部分 -->
                    <div v-if="hitTestingForm.retrieval_strategy === 'semantic'" class="flex items-center gap-2 mb-1.5">
                      <icon-pushpin />
    ...
    <!-- 检索设置模态窗 -->
    <a-modal :visible="retrievalSettingModalVisible" hide-title :footer="false" modal-class="rounded-xl">
      <!-- 顶部标题 -->
      <div class="flex items-center justify-between">
        <div class="text-lg font-bold text-gray-700">检索设置</div>
        <a-button type="text" class="!text-gray-700" size="small" @click="hideRetrievalSettingModal">
          <template #icon>
            <icon-close />
          </template>
        </a-button>
      ...
      <a-form :model="retrievalSettingForm" @submit="saveRetrievalSetting" class="pt-6">
        <a-form-item field="retrieval_strategy" label="检索策略" label-align="left">
          <a-radio-group v-model:model-value="retrievalSettingForm.retrieval_strategy" default-value="semantic" :options="[
        ...
        <a-form-item field="k" label="最大召回数量">
          <div class="flex items-center gap-4 w-full pl-3">
            ...
            <a-input-number v-model:model-value="retrievalSettingForm.k" class="w-[80px]" :default-value="4" />
          </div>
        </a-form-item>
        <a-form-item field="score" label="最小匹配度">
          <div class="flex items-center gap-4 w-full pl-3">
            <a-slider v-model:model-value="retrievalSettingForm.score" :step="0.01" :min="0" :max="0.99" />
            <a-input-number v-model:model-value="retrievalSettingForm.score" class="w-[80px]" :min="0" :max="0.99"
              :step="0.01" :precision="2" :default-value="0.5" />
          </div>
        </a-form-item>
        <!-- 底部按钮 -->
        <div class="flex items-center justify-between">
