<template>
  <div class="p-6">
    ...
    <div class="flex items-center justify-between mb-6">
      <!-- 左侧搜索框 -->
      <a-input-search :default-value="route.query?.search_word || ''" placeholder="请输入关键词搜索文档"
        class="w-[240px] bg-white rounded-lg border-gray-200" @search="(value: string) => {
          router.push({
            path: route.path,
            query: {
              search_word: value,
    ...
    <div class="">
      <!-- 表格内容 -->
      <a-table hoverable height="500" :pagination="{
        ...
        defaultPageSize: 20,
        showTotal: true,
      }" :loading="loading" :data="documents" :bordered="{ wrapper: false }" @page-change="(page: number) => {
  router.push({
    path: route.path,
    query: {
      current_page: page,
  ...
  ">
        <template #columns>
          <a-table-column title="#" data-index="position" align="center" :width="80"
            header-cell-class="rounded-tl-lg !bg-gray-200 text-gray-700" cell-class="bg-transparent text-gray-700" />
          <a-table-column title="文档名" data-index="name" :width="400" header-cell-class="!bg-gray-200 text-gray-700"
          ...
          <a-table-column title="字符数" data-index="character_count" header-cell-class="!bg-gray-200 text-gray-700"
            cell-class="bg-transparent text-gray-700">
            <template #cell="{ record }">
              {{ (record.character_count / 1000).toFixed(1) }}k
            </template>
          </a-table-column>
          <a-table-column title="召回次数" data-index="hit_count" header-cell-class="!bg-gray-200 text-gray-700"
            cell-class="bg-transparent text-gray-700" />
          <a-table-column title="上传时间" data-index="created_at" header-cell-class="!bg-gray-200 text-gray-700"
            cell-class="bg-transparent text-gray-700">
            <template #cell="{ record }">
              {{ moment(record.created_at * 1000).format('YYYY-MM-DD HH:mm:ss') }}
            </template>
          </a-table-column>
          <a-table-column title="状态" data-index="enabled" header-cell-class="!bg-gray-200 text-gray-700"
            cell-class="bg-transparent text-gray-700">
            <template #cell="{ record }">
              <a-space>
                <div v-if="record.enabled" class="w-2 h-2 bg-green-500 rounded-sm border border-green-700"></div>
          ...
          <a-table-column title="操作" data-index="operator" header-cell-class="rounded-tr-lg !bg-gray-200 text-gray-700"
            cell-class="bg-transparent text-gray-700 !h-[40px]" :width="100">
            <template #cell="{ record, rowIndex }">
              <a-space :size="0">
                <template #split>
                  <a-divider direction="vertical" />
                </template>
                <a-tooltip position="left" v-if="record.status === 'error'" :content="`错误信息: ${record.error}`">
                  <a-switch size="small" type="round" :default-checked="false" disabled />
                </a-tooltip>
                <a-switch v-else size="small" type="round" :model-value="record.enabled" @change="(value) => {
                  handleUpdateEnabled(
                ...
                <a-dropdown position="br">
                  ...
                  <template #content>
                    ...
                    <a-doption class="!text-red-700" @click="() =>
      ...
      <!-- 更新文档名字模态窗 -->
      <update-document-name-modal :document_id="updateDocumentID" :dataset_id="String(route.params?.dataset_id)"
        v-model:visible="updateDocumentNameModalVisible"
        :on-after-update="() => loadDocuments(String(route.params?.dataset_id ?? ''), req)" />

      <!-- 召回测试模态窗 -->
      <hit-testing-modal v-model:visible="hitModalVisible" :dataset_id="String(route.params?.dataset_id)" />
    </div>
  </div>
</template>
...
import UpdateDocumentNameModal from '@/view/space/datasets/documents/components/UpdateDocumentNameModal.vue'
import HitTestingModal from '@/view/space/datasets/documents/components/HitTestingModal.vue'
