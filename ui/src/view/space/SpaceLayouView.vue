    ...
    <div class="pt-6 sticky top-0 z-20 bg-gray-50">
      ...
      <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex items-center gap-2">
          ...
          <router-link to="/space/workflows"
            class="rounded-lg text-gray-700 px-3 h-8 leading-8 hover:bg-gray-200 transition-all"
            active-class="bg-gray-100">
            工作流
          </router-link>
        ...
        <!-- 右侧搜索 -->
        <a-input-search v-model="searchWord" placeholder="输入关键词进行搜索" class="w-[240px] bg-white rounded-lg border-gray-300"
          @search="search" @press-enter="search(searchWord)" />
      </div>
    </div>
    <!-- :max-height="815" -->
    <!-- <a-list max-height="951" :bordered="false" :scrollbar="false"> -->
...
const searchWord = ref<any>(route.query?.search_word || '')
