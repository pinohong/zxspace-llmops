...
const outputs = computed(() => {
  ...
  // 3.2 结束节点存在且有实际输出则直接返回
  if (endNodeResult && endNodeResult.outputs && Object.keys(endNodeResult.outputs).length > 0) {
    return endNodeResult.outputs
  }

  // 3.3 结束节点无输出时，查找最后一个非start/end节点有实际输出的节点
  for (let i = nodeResults.value.length - 1; i >= 0; i--) {
    const item = nodeResults.value[i]!
    if (
      item.node_data.node_type !== 'start' &&
      item.node_data.node_type !== 'end' &&
      item.outputs &&
  ...
  // 3.4 如果结束节点存在但outputs为空，仍返回不空的null（表示运行成功但无输出）
...
const onSubmit = async ({ errors }: { errors: Record<string, ValidatedError> | undefined }) => {
  ...
  // 6.4 调用hooks发起请求
  await handleDebugWorkflow(props.workflow_id, form.value, async (event_response) => {
      ...
      <a-tab-pane key="output" title="输出">
        ...
        >
          <div class="flex items-center gap-2 text-green-500 font-semibold">
            <icon-loading />
            <div>工作流运行中({{ nodeResults.length }}个节点已执行)</div>
          </div>
          <div class="flex flex-col gap-1 max-h-[120px] overflow-y-auto">
            <div
              v-for="(result, idx) in nodeResults"
              :key="idx"
              class="text-xs text-gray-600 flex items-center gap-1"
            >
              <icon-check-circle-fill class="text-green-500 flex-shrink-0" />
              <span>{{ result.node_data?.title ?? '-' }}</span>
              <span class="text-gray-400">({{ (result.latency * 1000).toFixed(1) }}ms)</span>
            </div>
          </div>
        </div>
        <!-- 非运行时状态 -->
