...
<template>
  <div
    v-if="props.visible"
    id="llm-node-info"
    class="absolute top-0 right-0 bottom-0 w-[400px] border-l z-50 bg-white flex flex-col"
  >
    ...
    <div class="flex items-center justify-between gap-3 mb-2">
      ...
      <div class="flex items-center gap-1 flex-1">
        <a-avatar :size="30" shape="square" class="bg-emerald-400 rounded-lg flex-shrink-0">
          <icon-branch />
        </a-avatar>
        <a-input
    ...
    <div class="flex-1 overflow-y-auto min-h-0 px-3">
    <!-- 表单信息 -->
    <a-form id="node-info-form" size="mini" :model="form" layout="vertical" @submit="onSubmit">
      <!-- 输入参数 -->
      <div class="flex flex-col gap-2">
        ...
        <div v-for="(input, idx) in form?.inputs" :key="idx" class="flex items-center gap-1">
          ...
          <div class="w-[47%] flex-shrink-0 flex items-center gap-1 min-w-0">
            <a-input-tag
