...
export const getWebApp = (token: string) => {
  return http.get<GetWebAppResponse>(`/web-apps/${token}`)
}
...
export const webAppChat = (
  ...
  return http.ssePost(`/web-apps/${token}/chat`, { body: req }, onData)
}

// 停止与指定 WebApp 进行对话
export const stopWebAppChat = (token: string, task_id: string) => {
  return http.post<BaseResponse<any>>(`/web-apps/${token}/chat/${task_id}/stop`)
}

// 获取指定应用的会话列表
export const getWebAppConversations = (token: string, is_pinned: boolean = false) => {
  return http.get<GetWebAppConversationsResponse>(`/web-apps/${token}/conversations`, {
    params: { is_pinned },
  })
}
