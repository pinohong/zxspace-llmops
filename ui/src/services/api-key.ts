...
export const createApiKey = (req: CreateApiKeyRequest) => {
  return http.post<BaseResponse<any>>(`/openapi/api-keys`, { body: req })
}

// 删除API秘钥请求
export const deleteApiKey = (api_key_id: string) => {
  return http.post<BaseResponse<any>>(`/openapi/api-keys/${api_key_id}/delete`)
}

// 修改API秘钥请求
export const updateApiKey = (api_key_id: string, req: UpdateApiKeyRequest) => {
  return http.post<BaseResponse<any>>(`/openapi/api-keys/${api_key_id}`, { body: req })
}

// 修改API秘钥激活请求
export const updateApiKeyIsActive = (api_key_id: string, is_active: boolean) => {
  return http.post<BaseResponse<any>>(`/openapi/api-keys/${api_key_id}/is-active`, {
    body: { is_active },
  })
}
...
export const getApiKeysWithPage = (req: BasePaginatorRequest) => {
  return http.get<GetApiKeysWithPageResponse>(`/openapi/api-keys`, { params: req })
}
