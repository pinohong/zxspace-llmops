...
export const getConversationMessages = (
  ...
  return http.get<GetConversationMessagesWithPageResponse>(
...
export const deleteConversation = (conversation_id: string) => {
  return http.post<BaseResponse<any>>(`/conversations/${conversation_id}/delete`)
}

// 删除特定会话下的指定消息
export const deleteMessage = (conversation_id: string, message_id: string) => {
  return http.post<BaseResponse<any>>(`/conversations/${conversation_id}/messages/${message_id}`)
}

// 获取指定会话的名称
export const getConversationName = (conversation_id: string) => {
  return http.get<BaseResponse<{ name: string }>>(`/conversations/${conversation_id}/name`)
}

// 修改指定会话的名称
export const updateConversationName = (conversation_id: string, name: string) => {
  return http.post<BaseResponse<any>>(`/conversations/${conversation_id}/name`, { body: { name } })
}

// 置顶或取消置顶某个会话
export const updateConversationIsPinned = (conversation_id: string, is_pinned: boolean) => {
  return http.post<BaseResponse<any>>(`/conversations/${conversation_id}/is-pinned`, {
    body: { is_pinned },
  })
}
