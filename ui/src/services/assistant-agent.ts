...
export const assistantAgentChat = (
  ...
  return http.ssePost(`/assistant-agent/chat`, { body: { query, image_urls } }, onData)
}

// 停止与辅助Agent进行对话
export const stopAssistantAgentChat = (task_id: string) => {
  return http.post<BaseResponse<any>>(`/assistant-agent/chat/${task_id}/stop`)
}
...
export const getAssistantAgentMessagesWithPage = (
  ...
  return http.get<GetAssistantAgentMessagesWithPageResponse>(`/assistant-agent/messages`, {
    params: req,
  })
}
...
export const deleteAssistantAgentConversation = () => {
  return http.post<BaseResponse<any>>(`/assistant-agent/delete-conversation`)
}
