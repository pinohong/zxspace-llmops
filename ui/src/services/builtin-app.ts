...
export const getBuiltinAppCategories = () => {
  return http.get<GetBuiltinAppCategoriesResponse>(`/builtin-apps/categories`)
}

// 获取内置应用列表信息
export const getBuiltinApps = () => {
  return http.get<GetBuiltinAppsResponse>(`/builtin-apps`)
}

// 将指定 Agent 模板添加到工作区
export const addBuiltinAppToSpace = (builtin_app_id: string) => {
  return http.post<BaseResponse<{ id: string }>>(`/builtin-apps/add-builtin-app-to-space`, {
    body: { builtin_app_id },
  })
}
