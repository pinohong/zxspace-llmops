...
class TestApiToolHandler:
    ...
    @pytest.mark.parametrize("provider_id", [
        "58828cf9-e2c4-45d3-99c2-7317ef3df350",
    ...
    def test_get_api_tool_provider(self, provider_id, client):
        ...
        assert resp.status_code == 200
        if provider_id.endswith("0"):
            assert resp.json.get("code_executor") == HttpCode.SUCCESS
        elif provider_id.endswith("5"):
            assert resp.json.get("code_executor") == HttpCode.NOT_FOUND

    @pytest.mark.parametrize("provider_id, tool_name", [
        ("58828cf9-e2c4-45d3-99c2-7317ef3df350", "GetCurrentWeather"),
        ("3944eee4-9d5a-4ca5-91c1-e56654cbc1e5", "Imooc")
    ])
    ...
    def test_update_api_tool_provider(self, client, db):
        provider_id = "58828cf9-e2c4-45d3-99c2-7317ef3df350"
        data = {
