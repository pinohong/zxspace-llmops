...
class AppHandler:
    ...
    @login_required
    def get_publish_history_with_page(self, app_id: UUID):
    ...
    def delete_debug_conversation(self, app_id: UUID):
        ...
        return success_message("清空应用调试会话记录")
    ...
    def get_debug_conversation_messages_with_page(self, app_id: UUID):
        ...
        # 2.调用服务获取数据
        message, paginator = self.app_service.get_debug_conversation_messages_with_page(app_id, req, current_user)

        # 3.创建响应结构
        resp = GetDebugConversationMessagesWithPageResp(many=True)

        return success_json(PageModel(list=resp.dump(message), paginator=paginator))
