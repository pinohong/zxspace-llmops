...
from internal.core.tools.builtin_tools.providers.glmimage.glm_image import GLMImageTool
from internal.entity.ai_entity import OPTIMIZE_PROMPT_TEMPLATE
from internal.entity.app_entity import AppStatus, DEFAULT_APP_CONFIG, AppConfigType, GENERATE_ICON_PROMPT_TEMPLATE
...
class AppService(BaseService):
    ...
    def auto_create_app(self, name: str, description: str, account_id: UUID) -> None:
        ...
        # 2.创建GLMImageTool工具实例
        glm_image_tool = GLMImageTool(size="1024x1024", quality="hd")

        # 3.构建生成icon链
        @traceable(name="GLM-Image-生图")
        def _generate_icon(prompt: str) -> str:
            result = glm_image_tool._run(prompt)
            if "图片链接为: " in result:
                return result.split("图片链接为: ")[-1].strip()
    ...
    def get_app(self, id: UUID, account: Account) -> App:
        """根据传递的id获取应用的基础信息"""
        # 1.查询数据库获取应用基础信息
        app = self.get(App, id)
        # 2.判断应用是否存在
    ...
    def stop_debug_chat(self, app_id: UUID, task_id: UUID, account: Account) -> None:
        """根据传递的应用id+任务id+账号，停止某个应用的调试会话，中断流失事件"""
        # 1.获取应用信息并校验权限
        self.get_app(app_id, account)
        # 2.调用智能体队列管理器停止特定任务
    ...
    def _validate_draft_app_config(self, draft_app_config: dict[str, Any], account: Account) -> dict[str, Any]:
        ...
        # 1.校验上次的草稿配置中对应字段，至少拥有一个可以更新的配置
        acceptable_fields = [
            ...
            for parameter in model_entity.parameters:
                ...
                if parameter.required:
                    ...
                    if parameter_value is None:
                        ...
                        # 3.10.值非空则校验数据类型是否正确，不正确则设置默认值
                        if get_value_type(parameter_value) != parameter.type.value:
                            parameter_value = parameter.default
                else:
                    # 3.11 参数非必填，数据非空的情况下需要校验
                    if parameter_value is not None:
                        if get_value_type(parameter_value) != parameter.type.value:
                            parameter_value = parameter.default

                # 3.12 判断参数是否存在options，如果存在则数值必须在options中选择
                if parameter.options and parameter_value not in parameter.options:
                    parameter_value = parameter.default

                # 3.13 参数类型为int/float，如果存在min/max时候需要校验
        ...
        if "tools" in draft_app_config:
            ...
            if not isinstance(tools, list):
                raise ValidateErrorException("工具列表必须是列表类数据")
            # 6.2 tools的长度不能超过5
            if len(tools) > 5:
            ...
            for tool in tools:
                ...
                # 6.9 校验对应的工具是否存在，并且需要划分成builtin_tool和api_tool
        ...
        if "workflows" in draft_app_config:
            ...
            if len(workflows) > 5:
                raise ValidateErrorException("Agent绑定的工作流数量不能超过五个")
            ...
            workflows_sets = set([str(workflow_record.id) for workflow_record in workflow_records])
            draft_app_config["workflows"] = [workflow_id for workflow_id in workflows if workflow_id in workflows_sets]
