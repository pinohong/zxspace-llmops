...
from internal.model import (Account, EndUser, Message, Conversation)
    ...
    def chat(self, req: OpenAPIChatReq, account: Account):
        ...
        if req.conversation_id.data:
            ...
            if (
                ...
                raise ForbiddenException("该会话不存在，或者不属于该应用/终端用户/调试方式")
        else:
            # 6.如果不存在则创建会话信息
        ...
        # 16.根据strea,类型差异执行不同的代码
        if req.stream.data is True:
            agent_thoughts_dict = {}

            def handle_stream() -> Generator:
                """流失事件处理器，在python只要在函数内部是用了yield关键字，那么这个函数的返回值类型肯定是生成器"""
                for agent_thought in agent.stream(agent_state):
                    # 提取thought以及answer
                    event_id = str(agent_thought.id)

                    # 将数据填充到agent_thoughts中,便于存储到数据库服务中
                    if agent_thought.event != QueueEvent.PING:
                        # 除了agent_message数据为添加，其他均为覆盖
