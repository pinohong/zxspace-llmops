    ...
    def debug_workflow(self, workflow_id: UUID, inputs: dict[str, Any], account: Account) -> Generator:
        ...
        def handle_stream() -> Generator:
            ...
            try:
                ...
                status = WorkflowResultStatus.SUCCEEDED if succeeded else WorkflowResultStatus.FAILED
                self.update(workflow_result, **{
                    "status": status,
                    "state": node_results,
                    "latency": (time.perf_counter() - start_at),
                })
                latest_workflow = self.db.session.query(Workflow).filter(Workflow.id == workflow.id).one()
                self.update(latest_workflow, **{
                    "is_debug_passed": succeeded,
                })
    ...
    def _validate_graph(self, workflow_id: UUID, graph: dict[str, Any], account: Account) -> dict[str, Any]:
        ...
        for edge in edges:
            try:
                ...
                # 18[升级更新].校验边Edges里的边必须唯一(source+target+source_handle_id必须唯一)，适配意图识别节点
                if any(
                        (
