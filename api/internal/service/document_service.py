...
from internal.model import Dataset, Document, UploadFile, ProcessRule, Segment, Account
...
class DocumentService(BaseService):
    ...
    def get_document(self, dataset_id: UUID, document_id: UUID, account: Account) -> Union[Document, None]:
        ...
        if document.dataset_id != dataset_id or document.account_id != account.id:
            raise ForbiddenException("当前用户无权获取该文档")
    ...
    def delete_document(self, dataset_id: UUID, document_id: UUID, account: Account) -> Document:
        ...
        document = self.get(Document, document_id)
        if document is None:
            raise NotFoundException("该文档不存在，请核实重试")
        if document.dataset_id != dataset_id or document.account_id != account.id:
        ...
        if document.status not in [DocumentStatus.COMPLETED, DocumentStatus.ERROR]:
            raise FailException("当前文档处于不可以删除状态，请稍后重试")
    ...
    def update_document_enabled(
        ...
        if document is None:
            raise NotFoundException("该文档不存在，请核实后重试")
        if document.dataset_id != dataset_id or document.account_id != account.id:
            raise ForbiddenException("当前用户无权限修改知识库下的文档，请核实后重试")
        ...
        if document.enabled == enabled:
            raise FailException(f"文档状态修改错误，当前意识{'启用' if enabled else '禁用'}状态")
