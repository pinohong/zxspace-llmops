...
class OpenAPISchema(BaseModel):
    ...
    def validate_server(cls, server: str) -> str:
        ...
        if server is None or server == "":
            raise ValidateErrorException("server不能为空且必须是字符串")
        return server
    ...
    def validate_description(cls, description: str) -> str:
        ...
        if description is None or description == "":
            raise ValidateErrorException("description不能为空且必须为字符串")
        return description
    ...
    def validate_paths(cls, paths: dict[str, dict]) -> dict[str, dict]:
        ...
        # 4.遍历提取到的所有接口并校验信息，涵盖operationID唯一表示，parameters参数
        operation_ids = []
        for interface in interfaces:
            ...
            for parameter in interface["operation"].get("parameters", []):
                ...
                if not isinstance(parameter.get("name"), str):
                    raise ValidateErrorException("parameter.name参数为字符串且不为空")
