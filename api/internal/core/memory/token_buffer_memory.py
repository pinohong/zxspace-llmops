...
class TokenBufferMemory:
    ...
    def get_history_prompt_messages(
        ...
        """根据传递的etoken限制+消息条数限制获取指定的会话模型的历史消息列表"""
        # 1.判断会话模型是否存在，如果不存在则直接返回列表
    ...
    def get_history_prompt_text(
        ...
        # 1.根据传递的信息获取历史消息列表
        messages = self.get.get_history_prompt_messages(max_token_limit, message_limit)

        # 2.调用LangChain集成的get_bugger_string()函数将消息列表装换成文本。
